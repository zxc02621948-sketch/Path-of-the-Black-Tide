...
const RenderNotes = {
  ...
  _resonanceRelatedRules(id) {
    const rules = {
      ...
      dodeca_fate_dice: [
        ...
        ['押注', '賭命骰子設定後會持續到取消或改押注。押中會提高傷害，12 面骰押中時額外再加上本次骰面；懊悔時下次受擊流程受到的傷害每層 +30%，最多 2 層。'],
        ['自然骰', '自然骰 12 且觸發共鳴弱點時，會把命運骰爆發推到最高。'],
      ],
      dodeca_lucky_dice: [
        ['破綻', '幸運骰不命中原生弱點，主要吃破綻與破綻倍數。'],
        ['押注', '賭命骰子押注會持續到取消或改押注；12 面骰押中賭命時額外再加上本次骰面；懊悔仍會累積下次受擊風險，最多 2 層。'],
        ['幸運星', '保留幸運星的 6/12 觸發規則：攻擊骰最終為 6 時傷害 +2；攻擊骰最終為 12 時傷害 +4。'],
      ],
  ...
  _relicDisplay(relic = {}) {
    const overrides = {
      ...
      wager_dice: {
        ...
        fusedDesc: '可押注 4 個骰面；命中改為傷害 +5；懊悔懲罰仍最多 2 層。',
      ...
      lucky_star: {
        ...
        fusedDesc: '攻擊骰最終為 6 時傷害 +2；每場戰鬥前 2 次可將 1 或 2 的攻擊骰面改為 6。若使用 12 面骰，攻擊骰最終為 12 時傷害 +4；每場戰鬥前 2 次骰到 6 時有 50% 機率改為 12。',
      ...
      eagle_eye_feather: {
        ...
        fusedDesc: '主戰使用弓時，最終骰面為 5 或 6 時，可視為命中原生弱點並觸發弓追擊；命中後本場第一次弓追加攻擊，額外傷害 +3。',
        lore: ['羽毛仍記得天空的高度。'],
        locationHint: '普通聖物獎勵中可能出現。',
      },
      flaw_lens: {
  ...
  _weaponDisplay(weapon = {}) {
    const overrides = {
      ...
      bow: ['弓', '弓', '主戰時，命中原生弱點後，可追加攻擊一次。每次追加攻擊若再次命中原生弱點，可繼續追加攻擊，但追擊不會觸發敵人的原生弱點破除效果。每回合最多額外追擊 2 次。'],
      dagger: ['匕首', '匕', '主戰時，命中弱點與破綻時，額外 +2 傷害。'],
      battle_drum: ['戰鼓', '鼓', '主戰攻擊後，敲響戰鼓：接下來 2 次我方主戰攻擊 +1 攻擊。不可疊加，可重新敲響刷新次數。持鼓者主戰時，骰面附加傷害減半。'],
      healing_staff: ['祈癒杖', '+', '主戰時，本次攻擊無視敵人格檔；命中原生弱點後，全隊恢復 1 HP。'],
      katana: ['太刀', '劍', '主戰造成傷害時，施加 1 層傷口。每 1 層傷口使敵人受到的傷害提高 5%，通常上限 15 層。'],
      sword_plus: ['裁衡劍', '劍', '主戰時，最終骰面 1-3 傷害 +2；4 以上傷害 +4。劍系聖物加成：刺劍連擊每次額外保底 1 次；重劍氣勢的失手流失每場戰鬥可化解 1 次。'],
      bow_plus: ['逐星弓', '弓', '主戰時，命中原生弱點後，可追加攻擊一次。每次追加攻擊若再次命中原生弱點，可繼續追加攻擊，但追擊不會觸發敵人的原生弱點破除效果。每回合最多額外追擊 3 次。'],
      dagger_plus: ['影牙匕首', '匕', '主戰時，命中弱點與破綻時額外 +2 傷害。若本次攻擊未命中任何弱點或破綻，額外造成等同於最終骰面的傷害。'],
      battle_drum_plus: ['星盤戰鼓', '鼓', '主戰攻擊後，敲響戰鼓：接下來 3 次我方主戰攻擊 +1 攻擊，並在接下來 3 個回合開始時讓全體獲得 1 點格檔。不可疊加，可重新敲響刷新。持鼓者主戰時，骰面附加傷害減半。'],
  ...
  _gearDisplay(gear = {}) {
    const overrides = {
      ...
      bone_dice_bag: ['骨骰袋', '骰', '每場戰鬥前 2 次，攻擊骰 1/2/3 會翻為 6/5/4。若搏命者擲出 1 被翻為 6，會直接觸發骰面 6 的大贏效果。'],
    };
  ...
  _itemDisplay(item = {}) {
    const overrides = {
      herb_pack: ['草藥包', '草', '立即使用，恢復目標 30% 最大生命；隊伍有輔助時恢復 40%。'],
      wish_chest: ['祈願寶箱', '箱', '使用後可選擇休整、聖物、裝備、武器升級或黎明祈願。'],
      whetstone: ['磨刀石', '石', '下一次主戰攻擊 +1。'],
  ...
  _resonanceNotes() {
    return [
      ...
      },
      {
        ...
        desc: '同身：融合沉鐵劍鞘 + 銀蜂針。',
        body: '銀蜂針不再觸發刺劍連擊，改為強化重劍。重劍命中後，額外獲得 3 點氣勢；每 5 點既有氣勢，使本次重劍傷害 +1。',
