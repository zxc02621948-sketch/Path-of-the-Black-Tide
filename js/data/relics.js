const RELICS = [
  ...
  },
  {
    ...
    poolWeight: 0.5,
    fusable: true,
    desc: '戰鬥開始後，持有者可押注 3 個骰面；押注會持續，直到取消或改押注。若本次攻擊命中押注骰面，傷害 +4；若未命中，懊悔 +1 層，下次受擊流程受到的傷害每層 +30%，最多 2 層。',
    effect: { type: 'wager_dice', faces: 3, damageBonus: 4, missPenaltyRate: 0.30, maxMissStacks: 2 },
    fusedEffect: { type: 'wager_dice', faces: 4, damageBonus: 5, missPenaltyRate: 0.30, maxMissStacks: 2 },
    nightOnly: false,
    lore: [
  ...
  },
  {
    ...
    poolWeight: 0.5,
    fusable: true,
    desc: '攻擊骰最終為 6 時傷害 +2；每場戰鬥前 1 次可將 1 或 2 的攻擊骰面改為 6。',
  ...
  },
  {
    ...
    rarity: 'rare',
    fusable: true,
    requiredWeaponFamilies: ['sword'],
    desc: '持有者使用劍系武器主戰時，高骰視為重劍。重劍命中後，獲得 3 點氣勢，直到戰鬥結束；每 1 點氣勢使自身基礎攻擊 +1。',
  ...
  },
  {
    ...
    effect: { type: 'rapier_stance', maxRoll: 3, damageRate: 0.5, chanceStep: 20, maxFollowUps: 5, minChance: 0, guaranteedFollowUps: 1 },
    fusedEffect: { type: 'rapier_stance', maxRoll: 3, damageRate: 0.5, chanceStep: 10, maxFollowUps: 10, minChance: 10, guaranteedFollowUps: 1 },
    nightOnly: false,
    lore: [
  ...
  },
  {
    ...
    rarity: 'rare',
    fusable: true,
    requiredWeaponFamilies: ['bow'],
    desc: '主戰使用弓時，最終骰面為 6 時，可視為命中原生弱點並觸發弓追擊；此視為命中不會破壞原生弱點。',
    effect: { type: 'eagle_eye_feather', finalMin: 6, firstFollowUpDamageBonus: 0 },
  ...
  },
  {
    ...
    effect: { type: 'wound_damage_bonus', woundOnHit: 1, bonusBaseRate: 0.10, bonusStepWounds: 3, bonusStepRate: 0.05, bonusMaxRate: 0.25 },
    fusedEffect: { type: 'wound_damage_bonus', woundOnHit: 2, bonusBaseRate: 0.15, bonusStepWounds: 3, bonusStepRate: 0.05, bonusMaxRate: 0.35, woundMax: 20 },
    nightOnly: false,
    lore: [
...
const RELIC_FUSED_DESCS = {
  ...
  wager_dice: '戰鬥開始後，持有者可押注 4 個骰面；押注會持續，直到取消或改押注。若本次攻擊命中押注骰面，傷害 +5；若未命中，懊悔 +1 層，下次受擊流程受到的傷害每層 +30%，最多 2 層。',
  lucky_star: '攻擊骰最終為 6 時傷害 +2；每場戰鬥前 2 次可將 1 或 2 的攻擊骰面改為 6。若使用 12 面骰，攻擊骰最終為 12 時傷害 +4；每場戰鬥前 2 次骰到 6 時有 50% 機率改為 12。',
  exorcism_ring: '每天第一次探索骰必定成功：若骰面低於成功門檻，將最終骰面提高到該門檻。',
  iron_scabbard: '持有者使用劍系武器主戰時，高骰視為重劍。重劍命中後獲得氣勢，直到戰鬥結束；每 1 點氣勢使自身基礎攻擊 +1。未融合獲得 3 點氣勢，融合後改為 5 點氣勢。',
  silver_bee_pin: '\u6301\u6709\u8005\u4f7f\u7528\u528d\u7cfb\u6b66\u5668\u4e3b\u6230\u6642\uff0c\u4f4e\u9ab0\u8996\u70ba\u523a\u528d\u3002\u523a\u528d\u547d\u4e2d\u5f8c\u4fdd\u5e95 1 \u6b21\u9023\u64ca\uff0c\u4e4b\u5f8c\u89f8\u767c\u9023\u64ca\uff1a\u7b2c\u4e00\u6b21\u6a5f\u7387 90%\uff0c\u4e4b\u5f8c\u6bcf\u6b21\u9023\u64ca\u6a5f\u7387 -10%\uff0c\u6700\u4f4e 10%\uff1b\u9023\u64ca\u50b7\u5bb3\u70ba\u672c\u6b21\u50b7\u5bb3\u7684 50%\u3002',
  eagle_eye_feather: '主戰使用弓時，最終骰面為 5 或 6 時，可視為命中原生弱點並觸發弓追擊；此視為命中不會破壞原生弱點。每場戰鬥第一次由鷹眼羽飾觸發的弓追加攻擊，額外傷害 +3。',
  ...
  pain_splinter_badge: '主戰造成傷害時，附加 2 層傷口。攻擊有傷口的敵人時，最終傷害提高 15%；每 3 層傷口額外 +5%，最高 35%，並將傷口上限提高至 20。',
...
const ECHO_RELIC_SYSTEMS = [
  ...
  },
  {
    ...
    guardianText: '守護者每 2 回合重擲命運盤，但保留目前幸運面與厄運面的數量。攻擊前擲命運骰，命中幸運面時單體傷害 x2 並新增 1 個幸運面；命中厄運面時自身失去目前生命的四分之一，且本回合攻擊減半。命中守護者原生弱點可新增 1 個厄運面；幸運與厄運各最多 3 面。',
